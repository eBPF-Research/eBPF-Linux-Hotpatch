	...
	int group)
{
	...
	for (; *pat; pat = (*endpat == ',' ? endpat + 1 : endpat))
	{
		 ...
		 */
		brace_level = 0;
		for (endpat = pat; *endpat && (*endpat != ',' || brace_level || (endpat > pat && endpat[-1] == '\\')); ++endpat)
		{
			if (*endpat == '{')
				brace_level++;
			else if (*endpat == '}')
		...
		while ((ap = *prev_ap) != NULL)
		{
			if (ap->pat != NULL)
			{
				 ...
				 */
				if ((allgroups || ap->group == findgroup) && ap->patlen == patlen && STRNCMP(pat, ap->pat, patlen) == 0)
				{
					/*
